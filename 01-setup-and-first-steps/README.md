...
課程包含具有故意設計的程式碼品質問題範例檔案。讓我們審查其中一個：
...
> 💡 **`@` 是什麼？** `@` 符號告訴 Copilot CLI 讀取檔案。你將在第 02 章中學習所有相關內容。目前，只需完全按照所示方式複製指令即可。
...
這些指令在互動模式下運作。**先從這四個開始**——它們涵蓋了 90% 的日常使用：

| 指令 | 作用 | 何時使用 |
|---------|--------------|-------------|
| `/help` | 顯示所有可用指令 | 當你忘記指令時 |
| `/clear` | 清除對話並重新開始 | 當切換主題時 |
| `/plan` | 在編寫程式碼前規劃你的工作 | 用於更複雜的功能 |
| `/research` | 使用 GitHub 和網路資源進行深入研究 | 當你需要在編寫程式碼前調查某個主題時 |
| `/model` | 顯示或切換 AI 模型 | 當你想更改 AI 模型時 |
| `/exit` | 結束階段 | 當你完成時 |

入門就介紹到這裡！當你變得熟悉之後，可以探索其他指令。

> 📚 **官方文件**：[CLI 指令參考](https://docs.github.com/copilot/reference/cli-command-reference) 以獲取指令和旗標的完整清單。

<details>
<summary>📚 <strong>其他指令</strong> (點擊展開)</summary>

> 💡 上方的五個指令涵蓋了你日常使用的大部分內容。此參考供你準備好探索更多內容時使用。
...
> 💡 技能將在 [第 05 章](../05-skills/README.md) 詳細介紹。MCP 伺服器將在 [第 06 章](../06-mcp-servers/README.md) 詳細介紹。
...
|---------|--------------|
| `/model` | 顯示或切換 AI 模型 |
| `/delegate` | 將任務交給 GitHub 上的 Copilot 編碼代理程式 (雲端代理程式) |
| `/fleet` | 將複雜任務拆分為平行子任務以加快完成速度 |
| `/tasks` | 檢視背景子代理程式和分離的 Shell 階段 |
...
|---------|--------------|
| `/diff` | 審查目前目錄中所做的變更 |
| `/review` | 執行程式碼審查代理程式以分析變更 |
| `/research` | 使用 GitHub 和網路資源執行深度研究調查 |
...
| 指令 | 作用 |
|---------|--------------|
| `/allow-all` | 自動核准此階段的所有權限提示 |
| `/add-dir <directory>` | 將目錄新增至允許清單 |
| `/list-dirs` | 顯示所有允許的目錄 |
| `/cwd`, `/cd [directory]` | 檢視或更改工作目錄 |

> ⚠️ **謹慎使用**：`/allow-all` 會跳過確認提示。對於信任的專案很棒，但處理不受信任的程式碼時要小心。

### 階段 (Session)

| 指令 | 作用 |
|---------|--------------|
| `/resume` | 切換到不同的階段 (可選指定階段 ID) |
| `/rename` | 重新命名目前階段 |
| `/context` | 顯示內容視窗權杖 (token) 使用情況與視覺化 |
| `/usage` | 顯示階段使用指標與統計數據 |
| `/session` | 顯示階段資訊與工作區摘要 |
| `/compact` | 摘要對話以減少內容使用量 |
| `/share` | 將階段匯出為 markdown 檔案或 GitHub gist |
...
|---------|--------------|
| `/help` | 顯示所有可用指令 |
| `/changelog` | 顯示 CLI 版本的更新紀錄 |
| `/feedback` | 向 GitHub 提交回饋 |
| `/theme` | 檢視或設定終端機主題 |

### 快速 Shell 指令

透過前綴 `!` 直接執行 Shell 指令而不使用 AI：
...
Copilot CLI 支援來自 OpenAI、Anthropic、Google 等的多個 AI 模型。你可以使用的模型取決於你的訂閱級別和區域。使用 `/model` 查看你的選項並在它們之間切換：
...
> 💡 **提示**：某些模型比其他模型消耗更多的「進階請求」。標記為 **1x** 的模型 (如 Claude Sonnet 4.5) 是一個很好的預設選擇。它們功能強大且高效。倍率較高的模型會更快耗盡你的進階請求額度，因此請在真正需要時才使用它們。
...
1. 啟動互動階段：`copilot`
2. 要求 Copilot CLI 摘要該檔案：`@samples/book-app-project/utils.py What does each function in this file do?`
...
3. **程式化模式**用於自動化。不需要互動。
4. **四個基本指令** (`/help`、`/clear`、`/plan`、`/exit`) 涵蓋了大部分日常使用。
