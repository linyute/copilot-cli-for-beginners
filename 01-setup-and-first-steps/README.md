...
| 命令 | 功能 |
|------|------|
| `/clear` | 放棄目前會話（不儲存歷史）並開始新的對話 |
| `/compact` | 摘要會話以減少上下文使用量 |
| `/context` | 顯示上下文視窗的 token 使用情況與視覺化資訊 |
| `/new` | 結束目前會話（儲存至歷史以便搜尋/恢復）並開始新的會話 |
| `/resume` | 切換到其他會話（可選擇指定會話 ID） |
| `/rename` | 重新命名目前會話（不填則自動產生名稱） |
| `/rewind` | 開啟時間軸選擇器，回溯到會話的任一先前時點 |
| `/usage` | 顯示會話使用量與統計數據 |
