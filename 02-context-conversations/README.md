...
當你新增檔案和對話時，Copilot CLI 的 [內容視窗](../GLOSSARY.md#context-window) 會填滿。兩個指令可以幫助你保持控制：
...
> 💡 **何時使用 `/clear`**：如果你一直在審查 `books.py` 且想要切換到討論 `utils.py`，請先執行 `/clear`。否則，來自舊主題的陳舊內容可能會干擾回應。
...
|-----------|--------|-----|
| 開始新主題 | `/clear` | 移除無關內容 |
| 長對話 | `/compact` | 摘要紀錄，釋放權杖 |
| 需要特定檔案 | 使用 `@file.py` 而非 `@folder/` | 僅載入你需要的內容 |
| 達到限制 | 啟動新階段 | 獲得全新的 128K 內容空間 |
| 多個主題 | 每個主題使用 `/rename` | 容易恢復正確的階段 |

#### 大型程式碼庫的最佳實作

1. **具體一點**：使用 `@samples/book-app-project/books.py` 而非 `@samples/book-app-project/`
2. **主題間清除**：切換焦點時使用 `/clear`
