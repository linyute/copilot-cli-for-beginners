...
> 💡 **犯錯或想嘗試不同的方法？** 使用 `/rewind`（或按兩次 Esc）開啟**時間軸選擇器**，讓你回到對話中的任何較早時間點，而不僅限於最近的一點。當你重新導航時，Copilot CLI 會詢問你是否要僅恢復對話，還是也要撤銷 Copilot 所做的檔案變更——不需要 git 儲存庫。當你走錯方向並想往回走而不必完全重新開始時，這非常有用。
...
| 走錯方向 | `/rewind` | 將對話回復到較早的任何時間點（也可以選擇還原檔案） |
