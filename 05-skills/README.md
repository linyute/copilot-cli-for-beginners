...
你可以在同一則訊息中呼叫多個技能，而且技能斜線指令可以出現在提示的任何位置 —— 不必只放在開頭。當你想一次執行兩項不同檢查時，這非常有用：

```bash
> 檢查 @samples/book-app-project/book_app.py，使用 /code-checklist，並同時執行 /generate-tests

> 審查 auth 模組，先用 /security-audit，然後再用 /code-checklist 檢查結果
```

Copilot 會在同一回應中套用每一項指定的技能，讓你不必發送多則訊息。

> 💡 **提示**：將技能斜線指令放在語句中最自然的位置即可。你可以把它放在訊息的開頭、中間或結尾。
...
為了讓本機外掛程式目錄保持最新，請使用下列指令更新：

```bash
copilot plugin marketplace update
```

外掛程式可以打包多種功能。一個外掛程式可能包含相關的技能、代理程式與 MCP 伺服器設定，讓它們協同運作。
