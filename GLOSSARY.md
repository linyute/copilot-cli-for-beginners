...
一項功能，讓 Copilot CLI 能記住事實與偏好，並在所有工作階段之間持久保存，而不僅限於單一會話。與會話歷史（儲存特定對話）不同，記憶為全域性，會自動應用於未來的工作階段。透過斜線指令 `/memory` 管理（`/memory on`、`/memory off`、`/memory show`）。記憶可以設定為作用於你的使用者帳戶（在所有倉庫可見），或作用於特定倉庫（與協作者共享）。
